...
Does not work in Safari (due to lack of WebRTC support). Actually, it can work, if your disable `if (hasNativeRTCSupport ())` check in JS code, but it will ask users to install a third-party plugin (developed by Skylink), and it can be quite disturbing.
